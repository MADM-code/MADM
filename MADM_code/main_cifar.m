...
clc
res_all = [];
addpath(genpath('H:\特征缺失的异常检测\code五折\datacv'));
for data_num = 1:10
    %get_dnames;
    get_dnames_cifar;
    dnames = [dname];
    ...
    fold_run_pos = crossvalind('kfold',normal_data(1:num_data,dim),5);
    [num_data,dim] = size(outlier_data);
    fold_run_neg = crossvalind('kfold',outlier_data(1:num_data,dim),5);
    
    % R = ismissing(data_only);
        ...
        for run = 1:tot_run
            for f = 1:tot_fold
                ...
                for v = 1:length(v_array)
                    ...
                    %             labelval_GOCK = test_model(val_data,val_lbls,model,model2,v_array(v));
